...
bin_means <- mydata %>% group_by(bin,coverage) %>% summarize(expenses = mean(expenses)) %>% 
...
bin_means[1,2]*wt1 + bin_means[2,2]*wt2 + bin_means[3,2]*wt3 + bin_means[4,2]*wt4 + bin_means[5,2]*wt5
```
...
Now, the estimated effect of insurance has dropped to a \$269 **decrease** in expenditures. But notice how much our sample size has been reduced! 
...
This isn't much better in terms of sample size. Even with only three variables to match on, we are severely restricting our sample. 
