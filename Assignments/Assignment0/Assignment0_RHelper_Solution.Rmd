...
```
We find that the average value in `d` is 175,120, with a median of 5897.5 (what does that tell us about the distribution?). The standard deviation is quite large at 2608.57. 
...
```{r problem-6}
library(tidyverse)
mydata %>% filter(category_1 == "Total deaths") %>% group_by(week_no) %>% summarise(mean_deaths = mean(counts)) %>% knitr::kable()
```
