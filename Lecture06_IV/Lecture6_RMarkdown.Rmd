...
There are many ways to run 2SLS. The most common is `ivreg()` from the `AER` package (Another good option is `feols()` from `fixest`). A package can help you quickly return objects that are important to you without you writing tedious functions, and can do impoertant things like standard error corrections. Just make sure you know what's going on in the package (for example, by checking out \`?ivreg').
